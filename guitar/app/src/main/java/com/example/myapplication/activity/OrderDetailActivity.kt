...
import com.example.myapplication.adapter.OrderCartProductAdapter
...
class OrderDetailActivity : AppCompatActivity() {
    ...
    private lateinit var orderCartProductAdapter: OrderCartProductAdapter
                ...
                if (response.isSuccessful) {
                    ...
                    data?.let {order->
                        ...
                        if(order.status == OrderStatus.PENDING) {
                            order.id?.let { id -> setUpBtnDone(id, OrderStatus.CANCELED) }
                            btnDone.text = "Cancel"
                        } else if(order.status == OrderStatus.PROCESSING) {
                            btnDone.text = "Received"
                            order.id?.let { id -> setUpBtnDone(id, OrderStatus.COMPLETED) }
                        } else if(order.status == OrderStatus.COMPLETED) {
                            btnDone.visibility = View.GONE
                        } else {
                            btnDone.visibility = View.GONE
                        }
                        orderCartProductAdapter = OrderCartProductAdapter(mutableListOf(), order.status)
